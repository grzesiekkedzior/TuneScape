...
RadioList::RadioList(QObject *parent) : QObject{parent} {}

RadioList::RadioList(Ui::MainWindow *ui)
    : ui(ui), model(new QStandardItemModel(this)) {
    ...
    connect(ui->treeView, &QTreeView::clicked, this,
            &RadioList::onTreeViewItemClicked);
    // for list
    ...
    // &RadioList::onTableViewDoubleClicked);
    connect(ui->tableView, &QTableView::doubleClicked, this,
            &RadioList::setRadioImage);
    connect(ui->tableView, &QTableView::activated, this,
            &RadioList::setRadioImage);

    connect(ui->playPause, &QPushButton::clicked, this,
            &RadioList::onPlayPauseButtonCliced);
    connect(ui->next, &QPushButton::clicked, this,
            &RadioList::onNextButtonClicked);
    connect(ui->previous, &QPushButton::clicked, this,
            &RadioList::onPrevButtonClicked);
    connect(ui->stop, &QPushButton::clicked, this,
            &RadioList::onStopButtonClicked);
    connect(ui->tableView, &QTableView::clicked, this,
            &RadioList::onTableViewClicked);
    connect(ui->tableView, &QTableView::activated, this,
            &RadioList::tableViewActivated);
    connect(ui->horizontalVolumeSlider, &QSlider::sliderMoved, this,
            &RadioList::sliderMoved);
    connect(ui->horizontalVolumeSlider, &QSlider::valueChanged, this,
            &RadioList::sliderMoved);
    connect(&streamReader, &StreamReader::dataReceived, this,
            &RadioList::handleDataReceived);
    // connect(ui->record, &QPushButton::clicked, this,
    // &RadioList::setMp3FileName);
    connect(ui->record, &QPushButton::clicked, this,
            &RadioList::startStopRecord);
    connect(ui->serachInput, &QLineEdit::returnPressed, this,
            &RadioList::searchStations);
    connect(ui->favorite, &QPushButton::clicked, this,
            &RadioList::addRadioToFavorite);
    connect(this, &RadioList::playIconButtonDoubleClicked, this,
            &RadioList::handleIconPlayButtonDoubleClick);
    connect(this, &RadioList::allIconsLoaded, this,
            &RadioList::onAllIconsLoaded);
    connect(ui->themeButton, &QPushButton::clicked, this,
            &RadioList::setDarkMode);
    ...
    ui->tableView->verticalHeader()->setDefaultSectionSize(
        ui->tableView->fontMetrics().height() + 2);
}
...
void RadioList::markIconPlayingStation(int radioNumber) {
    ...
    qDebug() << "allIconsAddresses.size() "
             << jsonListProcesor.getIconAddresses().size();
    ...
    if (label) {
        ...
        //} else {
        // label->setStyleSheet("background-color: #deffdf; color: white;
        // font-weight: bold;");
        //}
    } else {
...
void RadioList::setMp3FileName() {
    if (isPlaying) {
        ...
        streamRecorder->setFileName(title, extention);
               ...
               iceCastXmlData->getIsFavoritePlaying()) {
        QString iceCastTitle =
            ui->icecastTable
                ->item(iceCastXmlData->getCurrentPlayingStation(), 0)
                ->text();
        QString iceCastExtention =
            ui->icecastTable
                ->item(iceCastXmlData->getCurrentPlayingStation(), 2)
                ->text()
                .toLower();
        iceCastExtention.erase(iceCastExtention.constBegin(),
                               iceCastExtention.constBegin() + 6);
    ...
    if (currentPlayListPlaying == currentPlaylistIndex &&
        ...
        if (getIsPlaying()) {
            ...
            if (buttonContainer) {
                QLabel *label = buttonContainer->findChild<QLabel *>();
                if (label) {
                    label->setStyleSheet("background-color: #deffdf; color: "
...
bool RadioList::getIsPause() const { return isPause; }

void RadioList::setIsPause(bool newIsPause) { isPause = newIsPause; }

QSharedPointer<StreamRecorder> RadioList::getStreamRecorder() const {
    return streamRecorder;
}

bool RadioList::getIsDarkMode() const { return isDarkMode; }

void RadioList::setIsDarkMode(bool newIsDarkMode) {
    isDarkMode = newIsDarkMode;
}

RadioAudioManager &RadioList::getRadioManager() { return radioManager; }

bool RadioList::getIsPlaying() const { return isPlaying; }

void RadioList::setIsPlaying(bool newIsPlaying) { isPlaying = newIsPlaying; }
...
void RadioList::loadRadioIconList() {
    if (jsonListProcesor.isConnected) {
        ...
        for (int row = 0; row < dataSize; ++row) {
            // Without this condition app will crash?! Some check analyzer
            // suggest that it's unnecessary
            if (dataSize > row) {
...
void RadioList::handleNetworkReply(QNetworkReply *reply, int row) {
    ...
    QPushButton *button = qobject_cast<QPushButton *>(
        itemContainer->layout()->itemAt(0)->widget());
    if (!button) {
        reply->deleteLater();
        return;
    }
...
void RadioList::loadRadioList() {
    ...
    int dataSize = jsonListProcesor.getTableRows().size();
    // int batchSize = 50;
    //     for (int row = 0; row < qMin(loadedStationsCount + batchSize,
    //     dataSize);
    //          ++row)
    for (int row = 0; row < dataSize; ++row) {
        QList<QStandardItem *> rowItems;
        rowItems.append(
            new QStandardItem(jsonListProcesor.getTableRows().at(row).station));
        rowItems.append(
            new QStandardItem(jsonListProcesor.getTableRows().at(row).country));
        rowItems.append(
            new QStandardItem(jsonListProcesor.getTableRows().at(row).genre));
        rowItems.append(new QStandardItem(
            jsonListProcesor.getTableRows().at(row).stationUrl));
...
void RadioList::setTopListOnStart() {
    ...
    ui->treeView->selectionModel()->select(topIndex,
...
void RadioList::removeRadio(const QString data, const QString playlist) {
    ...
    // swap file
    if (QFile::remove(playlist) &&
        QFile::rename("temp_playlist.txt", playlist)) {
        qDebug() << "Correct" << data;
    } else {
        qDebug() << "Error " << data;
    }
...
void RadioList::loadAllData() {
    QStringList endpoints = {JSON_ENDPOINT_TOP,
                             ...
                             JSON_ENDPOINT_POPULAR, JSON_ENDPOINT_NEW};
...
void RadioList::onTreeViewItemClicked(const QModelIndex &index) {
    ...
    if (!checkItem(item, LIBRARY_TREE)) {
        if (!checkItem(item, "Discover") &&
            ui->tabRadioListWidget->currentIndex() == 2) {
...
void RadioList::playStream(int radioNumber) {
    ...
    currentRadioPlayingAddress =
        jsonListProcesor.getStreamAddresses(radioNumber);
...
void RadioList::setIndexColor() {
    customColor.reset(
        new CustomColorDelegate(radioIndexNumber, QColor(222, 255, 223), this));
    ...
    if (reply->error() == QNetworkReply::NoError) {
        ...
        if (contentTypeString.startsWith("image/")) {
            ...
            if (isDarkMode)
                ...
                ui->infoLabel->setPixmap(
                    QPixmap(":/images/img/radio-10-96.png"));
            ui->infoLabel->show();
        }
    } else {
...
void RadioList::onTableViewDoubleClicked(const QModelIndex &index) {
    if (jsonListProcesor.checkInternetConnection()) {
        ...
        ui->infoData->clear();
        if (isPlaying) {
            radioInfo->loadEndpoint(jsonListProcesor.getTableRows()
                                        .at(currentStationIndex)
                                        .station);
...
void RadioList::onPlayPauseButtonCliced() {
    ...
    if (ui->tabRadioListWidget->currentIndex() == 2 &&
        ...
        if (radioManager.getMediaPlayer()->isPlaying()) {
            ...
            currentPlayListPlaying = currentPlaylistIndex;
            playStream(radioIndexNumber);
            radioInfo->loadEndpoint(
                jsonListProcesor.getTableRows().at(radioIndexNumber).station);
            radioInfo->processInfoJsonQuery();
            radioInfo->setDataOnTable();
            QModelIndex newIndex = ui->tableView->model()->index(0, 0);
        ...
        ui->playPause->setIcon(QIcon(getIsPlaying()
...
void RadioList::clearTableViewColor() {
    ...
    //         if (isDarkMode) {
    //             model->setData(index, QColor(60, 60, 60),
    //             Qt::BackgroundRole); model->setData(index, QColor(Qt::white),
    //             Qt::ForegroundRole);
    //         } else {
...
void RadioList::onStopButtonClicked() {
    // TODO when internet connection is lost then stop button is unable to turn
    // off music!!! It can irritate some users but it is very rare case
    if (radioManager.getMediaPlayer()->isPlaying() &&
    ...
    if (getIsPlaying() == false &&
        ...
        if (radioManager.getMediaPlayer()->isPlaying()) {
            if (radioPlaylistCurrentPlaying < allTableRows.size() &&
                radioIndexCurrentPlaying <
                    allTableRows[radioPlaylistCurrentPlaying].size()) {
                QString data =
                    allIconsAddresses[radioPlaylistCurrentPlaying].at(
                        radioIndexCurrentPlaying) +
                    "," +
                    allStreamAddresses[radioPlaylistCurrentPlaying].at(
                        radioIndexCurrentPlaying) +
                    "," +
                ...
                if (isRadioAdded(data, "playlist.txt")) {
                    ...
                    ui->favorite->setIcon(
                        QIcon(":/images/img/bookmark-empty.png"));
                    ...
                    file.close();
                    ui->favorite->setIcon(
                        QIcon(":/images/img/bookmark-file.png"));
                }
            }
            setFavoriteStatons();
        }
...
void RadioList::handleDataReceived(const QString &data) {
    ...
    if (titleStart != -1 && titleEnd != -1) {
        QString title =
            metaData.mid(titleStart + 13, titleEnd - (titleStart + 13));
        qDebug() << title;
        ui->infoData->clear();
...
void RadioList::searchStations() {
    ...
    //**********************************************************
    if (allTableRows.size() > 4 && allIconsAddresses.size() > 4 &&
